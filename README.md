...
`plc` is a plain-text note-taking CLI. Every subcommand *creates or resolves a
file and prints its path* — it never opens an editor and is never interactive;
...
same store — think of it as the *Debit & Credit* app's simplicity with
...
tagged `[[ledger]]`. A *transaction* is a single line beginning with `$`. There
...
€2400 was drawn *from* your employer into your accounts. `book : 0.00 ✓`
...
Every field except the amount and one account is optional. In order:
  ...
  * **timestamp** — `YYYY-MM-DD HH:MM:SS ±ZZZZ`, the same format as the note
    stamp line. `plc fin add` stamps *now* by default; omit it and the
...
`plc fin add` writes into *today's* file and stamps *now*. Override the instant
...
Ask *"is this a place my money actually lives?"* If yes it is an `@` account
(you could count it); if it is only *what the money was for*, it is a `#`
